...
coord_x = tk.StringVar(value="100")
coord_y = tk.StringVar(value="100")
...
label_coords = tk.Label(root, text=f"Coordonnées : ({coord_x.get()}, {coord_y.get()})")
label_coords.pack()
...
point_id_main = main_canvas.create_oval(int(coord_x.get()) - 5, int(coord_y.get()) - 5,
                                        int(coord_x.get()) + 5, int(coord_y.get()) + 5,
                                        fill="red")
